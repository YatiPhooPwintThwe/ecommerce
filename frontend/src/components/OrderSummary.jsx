  ...
  return (
    <div className="space-y-4 rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
      ...
      <button
        onClick={onCheckout}
        disabled={loading}
        className="w-full rounded-lg bg-blue-400 px-5 py-2.5 text-sm font-medium text-white hover:bg-blue-700"
      >
      ...
      <div className="text-center">
        <a
          href="/"
          className="text-sm text-black-700 underline hover:no-underline"
        >
          Continue Shopping →
        </a>
      </div>
...
function Row({ label, value, bold, dim }) {
  return (
    <dl className="flex items-center justify-between">
      <dt
        className={`text-sm ${
          bold ? "font-semibold" : dim ? "text-slate-500" : "text-slate-700"
        }`}
      >
        {label}
      </dt>
